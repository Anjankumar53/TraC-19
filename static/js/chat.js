;(async () => {
	const qa = await fetch('static/qa.json').then(res => res.json())
	...
	function firstBotMessage() {
		let firstMessage =
			'Hello there!!Please answer the following questions to analyse your current medical situation'
	...
	function displayScore() {
		...
		if (totalScore > 100) {
			sendFromBot(
				'Condition:Highly risky,Advised to get a covid check-up as soon as possible'
			)
			...
			sendFromBot(
				'Condition:Least possibilty threat,Still advised to contact physician if symptoms are observed.'
			)
			sendFromBot('Refresh to retake the test')
			return
		}
